  ...
  return (
    ...
    >
      <div
        ...
        data-side={side}
        className={cn(
          "relative flex h-full w-(--sidebar-width) flex-col bg-sidebar transition-[width] duration-200 ease-linear",
...
function SidebarMenuButton({
  ...
  // Use native title tooltips only — Base UI TooltipTrigger causes SSR/client
  // hydration mismatches (id / data-base-ui-tooltip-trigger).
  const nativeTitle =
    mounted &&
    state === "collapsed" &&
    !isMobile &&
  ...
  const renderWithTitle =
    render && nativeTitle && React.isValidElement(render)
      ? React.cloneElement(
          render as React.ReactElement<{ title?: string }>,
          { title: nativeTitle },
        )
      : render
    ...
    props: mergeProps<"button">(
      {
        ...
        title: !renderWithTitle ? nativeTitle : undefined,
      },
      props
    ),
    render: renderWithTitle,
    state: {
